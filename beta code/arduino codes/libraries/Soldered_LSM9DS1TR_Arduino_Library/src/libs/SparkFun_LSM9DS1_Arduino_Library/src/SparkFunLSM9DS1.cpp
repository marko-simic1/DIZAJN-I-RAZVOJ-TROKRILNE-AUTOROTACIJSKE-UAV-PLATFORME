...
void LSM9DS1::initMag()
{
    ...
    // ST - Self-test enable
    if (settings.mag.tempCompensationEnable)
        tempRegValue |= (1 << 7);
...
void LSM9DS1::constrainScales()
{
    ...
    if ((settings.accel.scale != 2) && (settings.accel.scale != 4) && (settings.accel.scale != 8) &&
        (settings.accel.scale != 16))
    {
        settings.accel.scale = 2;
    }

    if ((settings.mag.scale != 4) && (settings.mag.scale != 8) && (settings.mag.scale != 12) &&
        (settings.mag.scale != 16))
    {
        settings.mag.scale = 4;
    }
...
uint8_t LSM9DS1::SPIreadBytes(uint8_t csPin, uint8_t subAddress, uint8_t *dest, uint8_t count)
{
