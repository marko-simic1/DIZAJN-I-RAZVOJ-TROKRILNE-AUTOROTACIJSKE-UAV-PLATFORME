...
# Ispis i snimanje primljenih podataka
def serial_receive_loop(ser):
    with open("imu_rec.csv", "a") as f:  ### DODANO — otvaranje datoteke
        while True:
            if ser.in_waiting:
                ...
                if data:
                    ...
                    if data.startswith("IMU:"):
                        # Parsiraj podatke nakon "IMU:"
                        line = data.replace("IMU:", "").strip()
                        f.write(line + "\n")  ### DODANO — zapis u CSV
                        f.flush()             ### DODANO — osiguraj da se odmah upiše
