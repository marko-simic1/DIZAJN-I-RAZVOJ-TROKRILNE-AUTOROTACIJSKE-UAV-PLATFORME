...
gyro_data = raw_rest(:,5:7);  % gx, gy, gz
...
[res_acc, ~] = fmincon(@(x)accError(x, acc_data), x0, [],[],[],[], lb, ub, [], options);
...
[res_gyro, ~] = fmincon(@(x)gyroError(x, gyro_data), x0, [],[],[],[], lb, ub, [], options);

xg = res_gyro;
A_gyro = [xg(1) xg(2) xg(3);
          xg(2) xg(4) xg(5);
          xg(3) xg(5) xg(6)];
b_gyro = [xg(7) xg(8) xg(9)];

fprintf('\n=== Žiroskop kalibracija ===\n');
disp('Matrica A:'); disp(A_gyro);
disp('Offset b:'); disp(b_gyro);

%% Kalibracija magnetometra
mag_data_swizzled = [mag_data(:,2), mag_data(:,1), -mag_data(:,3)]; 
...
%% Funkcije pogreške
function err = accError(x, data)
    A = [x(1) x(2) x(3);
         x(2) x(4) x(5);
         x(3) x(5) x(6)];
    b = [x(7) x(8) x(9)];
    data_corr = (data - b) * A';
    norms = sqrt(sum(data_corr.^2, 2));
    err = mean((norms - 1).^2); % idealna norma = 1g
end

function err = gyroError(x, data)
    A = [x(1) x(2) x(3);
         x(2) x(4) x(5);
         x(3) x(5) x(6)];
    b = [x(7) x(8) x(9)];
    data_corr = (data - b) * A';
    err = mean(sum(data_corr.^2, 2)); % idealna vrijednost = 0 (miruje)
end
